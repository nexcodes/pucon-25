...
export function EmissionsResult({ activities, results, totalEmissions }: EmissionsResultProps) {
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("desc")

  const sortedActivities = [...activities].sort((a, b) => {
    const emissionA = results.find((r) => r.activityId === a.date.getTime())?.emissions || 0
    const emissionB = results.find((r) => r.activityId === b.date.getTime())?.emissions || 0

    return sortOrder === "desc" ? Math.abs(emissionB) - Math.abs(emissionA) : Math.abs(emissionA) - Math.abs(emissionB)
  })
  ...
  const getActivityName = (activity: Activity) => {
    switch (activity.type) {
      case "transport":
        return `Car travel (${activity.data.distance} km, ${activity.data.fuelType})`
      case "electricity":
        return `Electricity (${activity.data.kWh} kWh)`
      case "gas":
        return `Gas (${activity.data.amount} ${activity.data.unit})`
      case "food":
        return `Food (${activity.data.grams}g of ${activity.data.foodType})`
      case "tree":
        return `Tree planting (${activity.data.trees} trees)`
      default:
  ...
  const chartData = activities.map((activity) => {
    const result = results.find((r) => r.activityId === activity.date.getTime())
    return {
      name: activity.type,
      emissions: result ? result.emissions : 0,
    }
  })
  ...
  return (
    <div className="space-y-8">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-2xl font-bold">{(totalEmissions / (activities.length || 1)).toFixed(2)} kg</div>
                ...
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Badge
                      ...
                      className={result.emissions < 0 ? "bg-green-100 text-green-800 hover:bg-green-100" : ""}
                    >
